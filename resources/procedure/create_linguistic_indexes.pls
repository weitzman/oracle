create or replace procedure create_linguistic_indexes
as

    procedure create_linguistic_index(p_index_name varchar2, p_unique varchar2, p_table varchar2)
    as
      v_ddl varchar2(32767);
    begin
    
         dbms_output.put_line('create ling idx for: '||p_index_name);
      ...
      v_ddl:= 'create '||p_unique||' index "'||identifier.get_for(p_index_name||'_L')||'" on "'||p_table||'" ('||substr(v_ddl,2)||') tablespace drupal_idx_ling';
      ...
      loop
        ...
        for c_typ in (select data_type from user_tab_columns where table_name= c_col.table_name and column_name= c_col.column_name) loop
           ...
           if c_typ.data_type= 'VARCHAR2' then
            ...
            create_linguistic_index(c_ind.index_name, replace(c_ind.uniqueness,'NONUNIQUE',''),c_ind.table_name);
